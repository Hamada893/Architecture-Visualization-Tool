/**
 * Puter serverless worker for project save/list/get.
 * Deploy: upload this file to Puter, then "Publish as Worker" and set VITE_PUTER_WORKER_URL to that URL.
 * Redeploy after any change so CORS and logic apply (e.g. from localhost or your app origin).
 */
...
// Explicit OPTIONS for preflight so CORS works from browser (e.g. localhost)
const optionsResponse = () => new Response(null, { status: 204, headers: corsHeaders })
router.options('api/projects/save', optionsResponse)
router.options('api/projects/list', optionsResponse)
router.options('api/projects/get', optionsResponse)
